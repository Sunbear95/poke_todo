...
화면은 오늘의 기록, 할 일 상세, 할 일 추가, 하루 마감, 컬렉션으로 구성된다. 사용자는 앱에서 직접 할 일을 추가하고, 습관 항목에서는 콤보로 키울 선호 몬스터를 골라 등장 가중치를 높일 수 있으며, 하루 마감 뒤 포획한 몬스터를 컬렉션에서 확인한다. 상태는 메모리 기반 React 상태로 관리하며, Expo Go로 실제 휴대폰에서 바로 실행할 수 있다.
